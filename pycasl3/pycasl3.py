...
class CASL2:
    ...
    class ByteCode:
        ...
        def __str__(self):
            ...
            if 1 < len(self.code):
                s += "\n"
                try:
                    s += "{:04x}\t{:04x}".format(self.addr + 1, self.code[1])
                except TypeError:
                    s += "{:04x}\t{}".format(self.addr + 1, self.code[1])
            if 2 < len(self.code):
                s += "\n"
                try:
                    s += "{:04x}\t{:04x}".format(self.addr + 2, self.code[2])
                except TypeError:
                    s += "{:04x}\t{}".format(self.addr + 2, self.code[2])
            return s
    ...
    def dump(self, a_code):
        ...
        for c in a_code:
            ...
            # if 1 < len(c.code):
            #     print("{:04x}\t{:04x}".format(c.addr + 1, c.code[1]))
            # if 2 < len(c.code):
            #     print("{:04x}\t{:04x}".format(c.addr + 2, c.code[2]))
            # addr += len(c.code)

        print("\nDefined labels")
        labels = self.symbols.values()
        for i in sorted(labels, key=lambda x: x.lines):
            print(i)
    ...
    def assemble(self, filename):
        ...
        # print('-- First pass --', file=sys.stderr)
        ...
        try:
            code_list = [
                self.replace_label(code) for code in self.tmp_code if code is not None
            ]
        except self.Error as e:
            e.report()
            sys.exit()
    ...
    def is_valid_program(self):
        ...
        while True:
            if not self.is_START():
                raise self.Error(
                    self.current_line_number, self.current_src, "START is not found."
                )
            is_data_exist = False
            while True:
    ...
    def get_line(self):
        ...
        while True:
            ...
            if len(line) == 0:
                self.next_line = self.Instruction(
                    None, "EOF", None, self.current_line_number + 1, ""
                )
                return current
            #
    ...
    def replace_label(self, bcode):
                    ...
                    # サブルーチンの実行開始番地が指定されていた場合、gotoに書かれているラベルの番地にする
                        ...
                        if label in self.symbols.keys():
                            ...
                            raise self.Error(
                                bcode.line_number,
                                bcode.src,
                                'Undefined label "%s" was found.' % x.split(".")[1],
                            )
                        #
                    #
                else:
        ...
        return self.ByteCode(
            [conv(i, bcode) for i in bcode.code],
            bcode.addr,
            bcode.line_number,
            bcode.src,
        )
    ...
    def remove_comment(self, file):
        return [
            i
            for i in [(n + 1, line[:-1].split(";")[0]) for n, line in enumerate(file)]
            if len(i[1]) > 0
        ]
    ...
    def split_line(self, line, line_number):
        ...
        re_args = r"(\s+{}(\s*,\s*{}(\s*,\s*{})?)?)?".format(re_arg1, re_arg2, re_arg3)
        ...
        op = result.group("op")
        args = None
        if result.group("arg1") is not None:
            args = [result.group("arg1")]
            if result.group("arg2") is not None:
                args.append(result.group("arg2"))
            if result.group("arg3") is not None:
    ...
    def register_label(self, inst):
        ...
        if inst.label is not None:
            ...
            if label_name in self.symbols.keys():
                print(
                    'Line %d: Label "%s" is already defined.'
                    % (inst.line_number, inst.label),
                    file=sys.stderr,
                )
                sys.exit()
            #
            self.symbols[label_name] = self.Label(
                label_name, inst.line_number, self.filename, self.addr
            )
        #
        return
    ...
    def conv_adr(self, addr):
        if re.match("-?[0-9]+", addr) is not None:
            a = a2l(int(addr))
        elif re.match("#[A-Za-z0-9]+", addr) is not None:
            a = int(addr[1:], 16)
        elif re.match("[A-Za-z_][A-Za-z0-9_]*", addr) is not None:
            a = self.current_scope + "." + addr
        elif re.match("=.+", addr) is not None:
            a = addr
        return a
    ...
    def gen_additional_dc(self, x, n):
        ...
        self.additional_dc.append(
            self.ByteCode(
                code, self.symbols[label_name].addr, n, "{}\tDC\t{}".format(l, x[1:])
            )
        )
        ...
        try:
            ...
            if op_table[inst.op][0] == -100:
                if inst.label is None:
                    print(
                        "Line %d: Label should be defined for START."
                        % inst.line_number,
                        file=sys.stderr,
                    )
                    sys.exit()
                ...
                if self.start_found:
                    ...
                    if inst.args is not None:
                        self.symbols["." + inst.label].goto = self.conv_adr(
                            inst.args[0]
                        )
                    return None
                else:
                    ...
                    return self.ByteCode(
                        self.gen_code_start(inst.op, inst.args),
                        self.addr,
                        inst.line_number,
                        inst.src,
                    )
            ...
            bcode = self.ByteCode(
                self.gen_code_func[op_table[inst.op][1]](inst.op, inst.args),
                self.addr,
                inst.line_number,
                inst.src,
            )
            ...
            print(
                'Line %d: Invalid instruction "%s" was found.'
                % (inst.line_number, inst.op),
                file=sys.stderr,
            )
            sys.exit()
